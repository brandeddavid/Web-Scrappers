...
while count < 15:
    ...
    try:
        ...
        type = browser.find_element_by_xpath('//*[@id="Type"]/option[2]')
        type.click()
        time.sleep(3)

        country = browser.find_element_by_xpath('//*[@id="Country"]/option[35]')
        ...
        with open('Gas.csv', 'a', newline='') as file:

            headers = ['Name', 'Country', 'Status', 'Latitude', 'Longitude', 'Design Capacity', 'Plant Type', 'Plant Use', 'Primary Fuel', 'Secondary Fuel', 'Location', 'Power Grid', 'Year Commissioned', 'Owner', 'Per Owner', 'Ownership Type', 'EPC Contractor', 'Operating Company', 'Regulatory Authority', 'Financed By', 'Reference']
            ...
            try:
                ...
                primary_fuel = browser.find_element_by_xpath('//*[@id="Type_of_Fuel_rng1_Primary"]').get_attribute('value')
                #print(primary_fuel)

                secondary_fuel_dropdown = browser.find_element_by_xpath('//*[@id="Type_of_Fuel_enumfield_rng2_Secondary"]')
                secondary_fuel = Select(secondary_fuel_dropdown).first_selected_option.text
                #print(secondary_fuel)
