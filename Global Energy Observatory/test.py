...
while country_count < 67:
    ...
    try:
        ...
        country = browser.find_element_by_xpath('//*[@id="Country"]/option['+country_count+']')
        time.sleep(3)
        country.click()
        ...
        for name in plant.text.split('\n'):

            power_plants.append(name)

        browser.close()

        count = 1

        while count < len(power_plants)+1:
            ...
            try:
                ...
                with open('test.csv', 'a', newline='') as file:
                    ...
                    headers = ['Name', 'Country', 'Status', 'Latitude', 'Longitude', 'Design Capacity', 'Plant Type',
                               'Plant Use', 'Location', 'Power Grid', 'Cooling Water', 'Year Commissioned', 'Owner',
                               'Per Owner', 'Ownership Type', 'EPC Contractor', 'Operating Company', 'Regulatory Authority',
                               'Financed By', 'Reference']
                    ...
                    try:
                        ...
                        design_capacity = browser.find_element_by_xpath(
                            '//*[@id="Design_Capacity_(MWe)_nbr"]').get_attribute('value')
                        ...
                        power_grid_dropdown = browser.find_element_by_xpath(
                            '//*[@id="Electric_Power_Grid_Connected_To_enumfield_rng1"]')
                        power_grid = Select(power_grid_dropdown).first_selected_option.text
                        # print(power_grid)

                        cooling_water = browser.find_element_by_xpath(
                            '//*[@id="Source_of_Cooling_Water_rng1"]').get_attribute('value')
                        # print(cooling_water)

                        year_comissioned = browser.find_element_by_xpath('//*[@id="Date_Commissioned_dt_1"]').get_attribute(
                            'value')
                        ...
                        ownership_type = browser.find_element_by_xpath('//*[@id="Type_of_Ownership"]').get_attribute(
                            'value')
                        # print(ownership_type)

                        epc_contractor = browser.find_element_by_xpath(
                            '//*[@id="Construction/EPC_Contractor"]').get_attribute('value')
                        # print(epc_contractor)

                        operating_company = browser.find_element_by_xpath('//*[@id="Operating_Company"]').get_attribute(
                            'value')
                        # print(operating_company)

                        regulatory_authority = browser.find_element_by_xpath(
                            '//*[@id="Regulatory_Authority"]').get_attribute('value')
                        ...
                        writer.writerow({'Name': name, 'Status': status, 'Country': country_name, 'Latitude': latitude,
                                         'Longitude': longitude, 'Design Capacity': design_capacity,
                                         'Plant Type': plant_type, 'Plant Use': plant_use, 'Location': location,
                                         'Power Grid': power_grid, 'Cooling Water': cooling_water,
                                         'Year Commissioned': year_comissioned, 'Owner': owner, 'Per Owner': per_owner,
                                         'Ownership Type': ownership_type, 'EPC Contractor': epc_contractor,
                                         'Operating Company': operating_company,
                                         'Regulatory Authority': regulatory_authority, 'Financed By': financed_by,
                                         'Reference': reference})
