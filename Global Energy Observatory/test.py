...
while count < 2:
    ...
    try:
        ...
        with open('Coal Plants.csv', 'a', newline='') as file:

            headers = ['Name', 'Country', 'Status', 'Latitude', 'Longitude', 'Design Capacity', 'Plant Type', 'Plant Use', 'Location', 'Power Grid', 'Cooling Water', 'Year Commissioned', 'Owner', 'Per Owner', 'Ownership Type', 'EPC Contractor', 'Operating Company', 'Regulatory Authority', 'Financed By', 'Reference']
            ...
            try:
                ...
                cooling_water = browser.find_element_by_xpath('//*[@id="Source_of_Cooling_Water_rng1"]').get_attribute('value')
                print(cooling_water)
                ...
                """
                number_of_owners = browser.find_element_by_xpath('//*[@id="NumberOfOwners"]').get_attribute('value')
                type(number_of_owners)
                owner_count = 1

                owner = []
                per_owner = []
                while owner_count < int(number_of_owners) + 1:

                    ownern = browser.find_element_by_xpath('//*[@id="Owners' + owner_count + '"]').get_attribute('value')
                    owner.append(ownern)
                    #print(ownern)

                    per_ownern = browser.find_element_by_xpath('//*[@id="%_Share_nbr' + owner_count + '"]').get_attribute('value')
                    per_owner.append(per_ownern)
                    #print(per_ownern)
                ...
                number_of_references = browser.find_element_by_xpath('//*[@id="NumberOfReferences"]').get_attribute('value')

                reference_count = 1

                while reference_count < number_of_references:

                    reference = browser.find_element_by_xpath('//*[@id="References'+ reference_count + '"]').get_attribute('value')
            ...
            """
                ...
                writer.writerow({'Name':name, 'Status':status, 'Country':country_name, 'Latitude':latitude, 'Longitude':longitude, 'Design Capacity':design_capacity, 'Plant Type':plant_type, 'Plant Use':plant_use, 'Location':location, 'Power Grid':power_grid, 'Cooling Water':cooling_water, 'Year Commissioned':year_comissioned, 'Owner':owner, 'Per Owner':per_owner, 'Ownership Type':ownership_type, 'EPC Contractor':epc_contractor, 'Operating Company':operating_company, 'Regulatory Authority':regulatory_authority, 'Financed By':financed_by, 'Reference':references})
